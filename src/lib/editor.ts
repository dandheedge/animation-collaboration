...
const getLastkeyFramedColor = (k, type) => {
  let colors = [];
  k.forEach((obj, i) => {
    const end = "e" in obj ? obj.e : [255, 255, 255, 0];
...
const getShapes = (json) => {
  console.log(json);
  return R.path(["layers"], json);
};

const findPaths = (json: any): (string | number)[][] => {
  const paths: (string | number)[][] = [];
  const stack: { obj: any; path: (string | number)[] }[] = [
    { obj: json, path: [] },
  ];
  ...
  while (stack.length > 0) {
    ...
    if (R.is(Object, obj)) {
      ...
      R.forEachObjIndexed((value, key) => {
        const keyAsNumber = !isNaN(Number(key)) ? Number(key) : key;
        stack.push({ obj: value, path: [...path, keyAsNumber] });
      }, obj);
    } else if (R.is(Array, obj)) {
      R.addIndex(R.forEach)((value, index) => {
        stack.push({ obj: value, path: [...path, index] });
      }, obj);
    }
  }
